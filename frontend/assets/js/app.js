...
async function sendComplaint() {
  ...
  if (!full_name) {
    showMessage(msgDiv, "⚠️ กรุณากรอก ชื่อ-นามสกุล ให้ครบก่อนส่ง", "error");
    return;
  }
  if (!phone_number) {
    showMessage(msgDiv, "⚠️ กรุณากรอก เบอร์โทรติดต่อ ให้ครบก่อนส่ง", "error");
    return;
  }
  if (!title) {
  ...
  try {
    ...
    showMessage(
      msgDiv,
      "⚠️ เชื่อมต่อเซิร์ฟเวอร์ไม่ได้ (เปิด Backend หรือยัง?)",
      "error",
    );
  } finally {
